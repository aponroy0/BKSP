...
export default function NoticeDetailCard({ notice }) {
  if (!notice) return null;

  const publishedDate = notice.createAt
    ? `${getMonth(notice.createAt)} ${getDay(notice.createAt)}, ${getYear(notice.createAt)}`
    : "";
  ...
  return (
    <article className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="py-8 px-6 sm:py-12 sm:px-16">
        ...
        {/* Content from Jodit editor */}
        {notice.content && (
          <div
            className="notice-content px-3"
